...
curl --basic -u admin:admin -X POST http://localhost:2480/command/ndm/sql -d "insert into _schemaMetadata (name, template) values ('tape', '{\"title\":[\"name\"], \"brief\":[\"tree\", \"shot\",\"expression\"], \"body\":[\"URI\"], \"links\":[]}')"
...
curl --basic -u admin:admin -X POST http://localhost:2480/command/ndm/sql -d "insert into _schemaMetadata (name, template) values ('shot',i'{\"title\":[\"name\", \"operators\"], \"brief\":[\"measurementTemperature\"], \"body\":[\"measurementAppliedField\", \"measurementAngle\"], \"links\":[]}' )"
